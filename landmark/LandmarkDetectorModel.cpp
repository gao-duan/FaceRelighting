...
string get_directory(const string& name) {
	...
	if (p != std::string::npos) {
		return std::string(name.begin(), name.begin() + p);
	}
}
// Constructors
...
void CLNF::Read_CLNF(string clnf_location)
{
	...
	string root = get_directory(clnf_location) + "/";
...
//	cout << "Reading the CLNF landmark detector/tracker from: " << main_location << endl;
	...
	if(!locations.is_open())
	{
		cout << "Couldn't open the model file, aborting "<<main_location << endl;
		return;
	}
	string line;
...
//	boost::filesystem::path root = boost::filesystem::path(main_location).parent_path();	
	string root = get_directory(main_location) + "/";
	/*TODO*/
